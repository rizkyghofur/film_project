...
class PopularMovieController extends GetxController {
  ...
  final popularMovieList = <Results>[].obs;
  RxBool isLoading = false.obs;
  Random random = Random();

  @override
  void onInit() {
    initDataPopularMovie();
    super.onInit();
  }
  ...
  initDataPopularMovie() async {
    EasyLoading.show(
        status: 'loading...',
        maskType: EasyLoadingMaskType.black,
        dismissOnTap: false);
    await theMovieDBService
        .getPopularMovies(pageNumber.toString())
        .then((response) {
      EasyLoading.dismiss();
      if (response.results!.isNotEmpty) {
        totalPage = response.totalPages!;
        popularMovieList.addAll(response.results!);
        update();
      }
    }).catchError((e) {
